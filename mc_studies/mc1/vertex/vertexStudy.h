...
// Wed Jul 31 12:16:42 2019 by ROOT version 6.10/04
...
struct TrackIde_t
{
  TrackIde_t(int tid, int gid, float e, TVector3 p)
   : trackId(tid), g4Id(gid), energy(e), pos(p)
  {}
  int trackId;
  int g4Id;
  float energy;
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Double_t        track_spt_idarr[32][1000][100];   //[ntracks_reco]
   Double_t        track_spt_earr[32][1000][100];   //[ntracks_reco]
   ...
   float GetIDENearVertex(const TVector3& pos, const float& radius); 
   void GetFirstInteractionInTPC(Vertex_t& point);
...
#ifdef vertexStudy_cxx
vertexStudy::vertexStudy(TTree *tree) : fChain(0) 
{
...
vertexStudy::~vertexStudy()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t vertexStudy::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t vertexStudy::LoadTree(Long64_t entry)
{
...
void vertexStudy::Init(TTree *tree)
{
...
void vertexStudy::Show(Long64_t entry)
{
...
Int_t vertexStudy::Cut(Long64_t entry)
{
