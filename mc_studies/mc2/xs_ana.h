...
// Mon Aug 12 10:09:38 2019 by ROOT version 6.10/04
...
struct Vertex_t
{
  ...
  void Dump()const{cout<<process<<" at ("<<position.X()<<","<<position.Y()<<","<<position.Z()<<")"<<endl;}
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   vector<vector<int> > *vertex_track_ids;
   vector<double>  *vertex_x;
   vector<double>  *vertex_y;
   vector<double>  *vertex_z;
   vector<vector<double> > *track_kink_x;
   vector<vector<double> > *track_kink_y;
   vector<vector<double> > *track_kink_z;
   ...
   TBranch        *b_vertex_track_ids;   //!
   TBranch        *b_vertex_x;   //!
   TBranch        *b_vertex_y;   //!
   TBranch        *b_vertex_z;   //!
   TBranch        *b_track_kink_x;   //!
   TBranch        *b_track_kink_y;   //!
   TBranch        *b_track_kink_z;   //!
   ...
   virtual void     Loop(int inDebug=0, int isMc=1);
   virtual Bool_t   Notify();
   virtual void     Show(Long64_t entry = -1);

   bool isInelastic_Alg2(int const& idMatchedTrk, TVector3& recobVertex);
   float getAngle(int const& idm, int const& ids);
   bool isInelastic_Alg1(int const& idMatchedTrk); 
   void getInteractionsInTpc();
   bool checkKink(int const& idMatchedTrk, TVector3& pos);
   void getPosNearKink(int const& idMatchedTrk, TVector3& beforeKinkPos, 
                                   const TVector3& kinkPos, TVector3& afterKinkPos); 
   void makeSmearingPlot(const std::vector<double>& trk_x,
   ...
   void doTrueXs(const TVector3& initialMom, 
                 const TVector3& firstPos, 
                 const TVector3& lastPos, 
                 const int& closest_vtx_id);
   void addUniformPoints(const TVector3& x0, const TVector3& xf, std::vector<TVector3>& orderedPoints);
   bool checkParticleInTpc();
   bool studyMc(const float& wcMom, const float& recoKinEn, const bool& isInel,
                const bool& isInel_1, const bool& isInel_2,
                const std::vector<double>& trk_x, const std::vector<double>& trk_y, const std::vector<double>& trk_z, const std::vector<double>& trk_p, const std::vector<double>& trk_dedx);
   void dumpEventInfo();
};
...
#ifdef xs_ana_cxx
xs_ana::xs_ana(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../piminusanatree.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../piminusanatree.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("../piminusanatree.root:/anatree");
...
Int_t xs_ana::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t xs_ana::LoadTree(Long64_t entry)
{
...
void xs_ana::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("vertex_track_ids", &vertex_track_ids, &b_vertex_track_ids);
   fChain->SetBranchAddress("vertex_x", &vertex_x, &b_vertex_x);
   fChain->SetBranchAddress("vertex_y", &vertex_y, &b_vertex_y);
   fChain->SetBranchAddress("vertex_z", &vertex_z, &b_vertex_z);
   fChain->SetBranchAddress("track_kink_x", &track_kink_x, &b_track_kink_x);
   fChain->SetBranchAddress("track_kink_y", &track_kink_y, &b_track_kink_y);
   fChain->SetBranchAddress("track_kink_z", &track_kink_z, &b_track_kink_z);
   fChain->SetBranchAddress("electron_lifetime", &electron_lifetime, &b_electron_lifetime);
   Notify();
}

Bool_t xs_ana::Notify()
{
...
void xs_ana::Show(Long64_t entry)
{
...
Int_t xs_ana::Cut(Long64_t entry)
{
