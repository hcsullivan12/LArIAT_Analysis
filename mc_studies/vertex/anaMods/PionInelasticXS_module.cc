...
private:
  ...
  /// Utitlity to dump vector more nicely
...
//----------------------------------------------------------------------------------------------------
// Compute angles
void PionInelasticXS::computeAngles(float& phi, float& theta, const TVector3& p0Hat)
{
  TVector3 zHat(0,0,1);
                                            ...
                                            const std::vector<art::Ptr<recob::Track> >& tracklist)
{
  ...
  */ 
  ...
  for(size_t iTrk = 0; iTrk < tracklist.size(); iTrk++)
  {
    ...
    {
      if (minId == 0) {theTracksLeaving.emplace(iTrk, false); hVertexDiff->Fill(diffArr[0]);}
      if (minId == 1) {theTracksLeaving.emplace(iTrk, true); hVertexDiff->Fill(diffArr[1]);}
    }
