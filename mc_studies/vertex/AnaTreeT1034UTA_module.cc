...
// File:        AnaTreeT1034UTA_module.cc
//
// Edited version of AnaTreeT1034_module
////////////////////////////////////////////////////////////////////////
...
private:
  ...
  std::vector<std::string> InteractionPointSubType; //<---Geant 4 Primary Interaction Type
...
lariat::AnaTreeT1034UTA::AnaTreeT1034UTA(fhicl::ParameterSet const &pset)
    : EDAnalyzer(pset), fCalorimetryAlg(pset.get<fhicl::ParameterSet>("CalorimetryAlg"))
{
  ...
  if (!isdata)
  {
    ...
    for (unsigned int i = 0; i < geant_part.size(); ++i)
    {
      ...
      if (geant_part[i]->Process() == pri)
      {
        ...
        InteractionPointSubType.push_back(subclassifier.Classify(plist, TrackId[i]));
        ...
        if (!thisTracjectoryProcessMap.size())
        {
          ...
          if (NumberDaughters[i])
          {
            ...
            for (unsigned int iD = 0; iD < geant_part.size(); ++iD)
            {
              ...
              {
                InteractionPointType.push_back(geant_part[iD]->Process());
              }
            }
          }
          else
        ...
        else
        {
          ...
          for (auto const &couple : thisTracjectoryProcessMap)
          {
            ...
            InteractionPoint.push_back(interestingPoint);
            if ((truetraj.KeyToProcess(couple.second)).find("hadElastic") != std::string::npos)
              InteractionPointType.push_back("hadElastic");
            if ((truetraj.KeyToProcess(couple.second)).find("pi-Inelastic") != std::string::npos)
              InteractionPointType.push_back("pi-Inelastic");
            if ((truetraj.KeyToProcess(couple.second)).find("pi+Inelastic") != std::string::npos)
              InteractionPointType.push_back("pi+Inelastic");
            if ((truetraj.KeyToProcess(couple.second)).find("CHIPSNuclearCaptureAtRest") != std::string::npos)
              InteractionPointType.push_back("CHIPSNuclearCaptureAtRest");
            if ((truetraj.KeyToProcess(couple.second)).find("Decay") != std::string::npos)
              InteractionPointType.push_back("Decay");
            if ((truetraj.KeyToProcess(couple.second)).find("CoulombScat") != std::string::npos)
              InteractionPointType.push_back("ColoumbScat");
            if ((truetraj.KeyToProcess(couple.second)).find("hBertiniCaptureAtRest") != std::string::npos)
              InteractionPointType.push_back("hBertiniCaptureAtRest");
          }
        }
