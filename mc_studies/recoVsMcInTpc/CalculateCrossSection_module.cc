...
void lariat::CalculateCrossSection::analyze(art::Event const &evt)
{
  ...
  for (auto recoTrk : tracklist)
  {
    ...
    if (recoTrk->ID() != matchedRecoTrkKey)
      continue;
    ...
    {
      if (recoTrkPileUp->ID() == matchedRecoTrkKey)
        continue; // Skip self track
    ...
    if (fmcal.isValid())
    {
      ...
      for (size_t j = 0; j < calos.size(); ++j)
      {
        ...
        for (size_t k = 0; k < calos[j]->dEdx().size(); ++k)
        {
          ...
          if (!isThisPointInActiveVolume)
            continue; // I'm filling the track only with points in active volume
        ...
        if (furtherstInZCaloPointIndex)
          isTrackInteracting = isWithinActiveVolume(calos[j]->XYZ()[furtherstInZCaloPointIndex].X(),
  ...
  if (recoPitch_v.size() != recoDEDX_v.size())
    throw cet::exception("Geometry") /// <== ... throw exception
        << "Different dimensions ... somethings going on, may I smell... \n";
  if (recoEDep_v.size() != recoResR_v.size())
    throw cet::exception("Geometry") /// <== ... throw exception
        << "Different dimensions ... somethings going on, may I smell... \n";
  if (recoPitch_v.size() != recoZPos_v.size())
    throw cet::exception("Geometry") /// <== ... throw exception
        << "Different dimensions ... somethings going on, may I smell... \n";
  if (recoPitch_v.size() != recoEDep_v.size())
    throw cet::exception("Geometry") /// <== ... throw exception
        << "Different dimensions ... somethings going on, may I smell... \n";
  ...
  else
  {
    ...
    for (size_t p = 0; p < plist.size(); ++p)
    {
      ...
      if (!(proc.find("primary") != std::string::npos))
        continue;
  ...
  for (auto inKE : recoIncidentKE_v)
    hRecoMCIncidentKE->Fill(inKE);
  if (isTrackInteracting && recoIncidentKE_v.size())
    hRecoMCInteractingKE->Fill(recoIncidentKE_v[recoIncidentKE_v.size() - 1]);

  EnLoss = calculatedEnLoss;
  recoKEFF = initialKE;
  recoInteractingKE = recoIncidentKE_v[recoIncidentKE_v.size() - 1];
  for (size_t i = 0; i < recoIncidentKE_v.size(); i++)
    recoIncidentKE[i] = recoIncidentKE_v[i];
...
void lariat::CalculateCrossSection::endJob()
{
  ...
  */
  ...
  for (int iBin = 1; iBin <= hRecoMCInteractingKE->GetNbinsX(); ++iBin)
  {
    ...
    if (hRecoMCIncidentKE->GetBinContent(iBin) == 0)
      continue; //Temporary fix to ensure that no Infinities are propagated to pad
...
void lariat::CalculateCrossSection::beginJob()
{
  ...
  for (size_t is = 0; is < vectorA.size(); is++)
    std::cout << vectorA[is] << " " << vectorB[is] << "\n";
